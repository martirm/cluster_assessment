...
cs_w <- function(G,s){ #sum of weight of edges connecting vertices of s to the rest of the graph
  sum(G[s,-s])/2 #sum the elements of rows in s and columns not in s (divide by two because each edge is counted twice)
}
...
clustering_coef_subgraph <- function(G,s,n_step=100, w_max=w_max){
  ...
  clustering_coef_w(Gs, n_step, w_max)
}
...
scoring_functions <- function(G,com,fix_labels=TRUE, globals_only=TRUE, no_clustering_coef=TRUE,
  ...
  for (i in 1:n_com){
    ...
    D[i,]<- c(internal_density_s(G,s),m_subgraph_w(G,s),average_degree(G,s),frac_over_median(G,s),expansion(G,s),
