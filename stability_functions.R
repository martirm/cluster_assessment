...
sourceCpp("resampled_edgelist.cpp")
...
perturbate_matrix <- function (M, sd=-1, sd_scale=0.1, truncated=FALSE){
    ...
    for (i in 1:(n-1)){
        for (j in (i+1):n){
            if (M[i,j] != 0){
                if (truncated) 
                    M[i,j] <- rtruncnorm(1, mean=M[i,j], sd=sd, a=0, b=1)
                else
                    M[i,j] <- rtruncnorm(1, mean=M[i,j], sd=sd, a=0)
...
cluster_bootstrap_p <- function (returns, n_samples=10, sd=-1, sd_scale=0.1, truncated=FALSE){
  #replace normal distribution with truncated normal (between 0 and 1)
    ...
    for (i in 1:n_samples){
        ...
        vi_distances[i] <- vi.dist(c_dist_nd,c_dist_resample)/log(n)
        rand_indices[i] <- rand.index(c_dist_nd, c_dist_resample)
...
cluster_bootstrap_igraph <- function (clust_alg, g, n_samples=10, sd=-1, sd_scale=0.1, truncated=FALSE, edgelist=TRUE, perturbate=TRUE, add_self_edges=TRUE){
    ...
    for (i in 1:n_samples){
        ...
        vi_distances[i] <- vi.dist(memb_nd, memb_resample)/log(n)
        rand_indices[i] <- rand.index(memb_nd, memb_resample)
...
resample_igraph <- function (g, S_sample, perturbate=FALSE, sd=-1, sd_scale=0.1, truncated=FALSE, percentile=0.95, add_self_edges=FALSE){
    ...
    if (perturbate){
        p <- function(weight){
            ...
            if (truncated) 
                return(rtruncnorm(1, mean=weight, sd=sd, a=0, b=1))
            else
                return(rtruncnorm(1, mean=weight, sd=sd, a=0))
        }
...
cluster_statistic_old <- function(data, sample, memb_original, clust_alg, g){
...
boot_cluster <- function(clust_alg, g, R=999, type="global"){
    ...
    b <- boot(vertex_list, cluster_statistic, R=R, memb_original=memb, 
              clust_alg=clust_alg, g=g, type=type)
}


walktrap_cut <- function(g, n=2){
    #applies walktrap hierarchical algorithm and cuts into n clusters
    c <- cluster_walktrap(g)
    c$membership <- cutat(c,n)
    return(c)
}

plot_sbm_colored <- function(g, block_sizes = c(25,25,25,25)){
    #Plots the graph coloring the vertices according to the "block_sizes" vector
    #(assuming communities are formed by consecutiver vertices and in the givern order)
    color_vector = as.vector(mapply(rep, 1:(length(block_sizes)), block_sizes))
    #color_vector = as.vector(mapply(rep,c("red","blue","green","yellow"), block_sizes))
    par(mar=c(0,0,0,0)+.1)
    plot(g, vertex.size=8,vertex.label=NA, vertex.color=color_vector)
}
...
boot_alg_list <- function (alg_list, g, R=999, return_data=FALSE, type="global"){
    #type can be "global" (VI and Rand) or "cluster-wise" (Jaccard)
    evaluate_boot_g <- function(clust_alg) boot_cluster(clust_alg, g, R=R, type=type)
    b <- lapply(clust_alg_list, evaluate_boot_g)
...
# b_karate <- boot_alg_list(alg_list, karate, return_data=TRUE) #non convergence for leading eigenvector (this was working before, for some reason)
# b_forex <- boot_alg_list(alg_list, g_forex, return_data=TRUE)
# b_news <- boot_alg_list(alg_list, g_small, return_data=TRUE)
# b_sbm <- boot_alg_list(alg_list, g_sbm, return_data=TRUE)
# plot_sbm_colored(g_sbm)
# 
# 
# b_rnd_karate <- boot_alg_list(alg_list, rewire_graph(karate), return_data=TRUE)
# b_rnd_forex <- boot_alg_list(alg_list, rewire_graph(g_forex), return_data=TRUE)
# b_rnd_news <- boot_alg_list(alg_list, g_small, return_data=TRUE)
# b_rnd_sbm <- boot_alg_list(alg_list, rewire_graph(g_sbm), return_data=TRUE) #non convergence for leading eigenvector (LOOK INTO THIS)
# 
# 
# b_combined_forex <- rbind(b_forex, b_rnd_forex)
# 
# 
# t <-sapply (b_karate, get_statistics)
# lapply(b_karate,boot.ci, index=c(2))
