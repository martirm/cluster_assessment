...
merge3 <- function(table1, table2, suffix="_r"){
    ...
    #the relative score is not well defined if either value is negative, so we leave those positions as NA
    for (i in 1:nrow(table1)){
        for (j in 1:ncol(table1)){
            if (is.na(table1[i,j]) | is.na(table2[i,j]) | table1[i,j] <= 0 | table2[i,j] <= 0){
                table3[i,j] <- NA
            }
        }
    }
...
evaluate_significance_r <- function(g, alg_list=clust_alg_list, no_clustering_coef=FALSE, 
    ...
    else if (table_style==4 & n_reps>1 ) results_table <- merge4(table1, table2, table_per)
    ...
    if (ground_truth){
        if (table_style==4) 
            ...
            results_table[,"ground truth"] <- table1[,"ground truth"]
    }
